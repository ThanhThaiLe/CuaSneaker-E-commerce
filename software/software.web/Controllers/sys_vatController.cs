...
{
    public partial class sys_vatController : common.ControllerBase.BaseAuthenticationController
    {
        private sys_vat_repo repo;
        private AppSetting _appsetting;
        public sys_vatController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
            repo = new sys_vat_repo(context);
            _appsetting = appsetting.Value;
        }
        public IActionResult getListUse()
        {
            var result = repo._context.sys_vats.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.name + "(" + q.value + ")",
                value = q.value,
        ...
        {
            var model = JsonConvert.DeserializeObject<sys_vat_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                DTResult<sys_vat_model> result = new DTResult<sys_vat_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
        ...
        public ActionResult downloadTemp()
        {
            ...
            string file = newpath + "\\sys_vat.xlsx";
            byte[] fileBytes = System.IO.File.ReadAllBytes(file);
            System.IO.File.WriteAllBytes(file, fileBytes);
            MemoryStream ms = new MemoryStream(fileBytes);
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, "sys_vat.xlsx");
        }
        [HttpPost]
