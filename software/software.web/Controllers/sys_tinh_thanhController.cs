...
{
    public partial class sys_tinh_thanhController : common.ControllerBase.BaseAuthenticationController
    {
        private sys_tinh_thanh_repo repo;
        private AppSetting _appsetting;
        public sys_tinh_thanhController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
        ...
        {
            var result = repo._context.sys_tinh_thanhs.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.ten,
            }).ToList();
