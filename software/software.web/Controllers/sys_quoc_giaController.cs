...
{
    public partial class sys_quoc_giaController : common.ControllerBase.BaseAuthenticationController
    {
        private sys_quoc_gia_repo repo;
        private AppSetting _appsetting;
        public sys_quoc_giaController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
            repo = new sys_quoc_gia_repo(context);
            _appsetting = appsetting.Value;
        }
        public IActionResult getListUse()
        {
            var result = repo._context.sys_quoc_gias.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.ten,
            }).ToList();
        ...
        {
            var model = JsonConvert.DeserializeObject<sys_quoc_gia_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                DTResult<sys_quoc_gia_model> result = new DTResult<sys_quoc_gia_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
