...
namespace software.repo.erp.Repo
{
    public class erp_nhap_kho_repo
    {
        ...
        public string generateName(erp_nhap_kho_model model)
        {
            ...
            var loai_nhap_xuat = _context.erp_loai_nhap_xuats.Where(q => q.id == model.db.loai_nhap).SingleOrDefault();
        ...
        {
            var db = await _context.erp_nhap_khos.Where(q => q.id == long.Parse(id)).SingleOrDefaultAsync();
