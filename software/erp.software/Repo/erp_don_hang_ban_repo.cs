...
namespace software.repo.erp.Repo
{
    public class erp_don_hang_ban_repo
    {
        ...
        public async Task<erp_don_hang_ban_model> getElementById(long id)
        {
        ...
        {
            var db = await _context.erp_don_hang_bans.Where(q => q.id == long.Parse(id)).SingleOrDefaultAsync();
