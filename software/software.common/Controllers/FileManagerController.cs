...
namespace software.common.Controllers
{
    ...
    public class FileManagerController : Controller
    {
        ...
        public FileManagerController(IOptions<AppSetting> appsetting)
        {
            _appsetting = appsetting.Value;
        }

        /// <summary>
        /// download file với tên file đã lưu path và tên file tại db
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>

        [HttpGet("DowloadFile")]
        public async Task<ActionResult> DowloadFile(string id)
        {
            var filedb = "láy dữ liệu của db muốn download để lấy path";
            var request = Request;
            ...
            var path_new = path + filedb;
            return new PhysicalFileResult(path_new, FileCommon.GetContentType(path_new)) { FileDownloadName = "Tên file" };
        }

        /// <summary>
        /// download folder thành file zipp
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>

        [HttpGet("DowloadFolder")]
        public async Task<ActionResult> DowloadFolder(string id)
        {
            ...
            {
                var file_path = list_file[i];
                var path = Path.Combine(_appsetting.folder_path, "file_upload", host);
                var path_new = path + file_path;
                var model = new ZipItem();
                model.file_name = file_path;
                model.file_path = file_path;
                model.doc_no = "Ten folder";
                zipItems.Add(model);
            }
            var path1 = Path.Combine(_appsetting.folder_path, "file_upload", "zip");
            if (!Directory.Exists(path1))
            {
                Directory.CreateDirectory(path1);
            }
            var webRoot = _appsetting.folder_path;
            var tick = DateTime.Now.Ticks;
            var fileName = host + tick + "." + "zip";
            ...
            {
                foreach (var fileZip in zipItems)
                {
                    archive.CreateEntryFromFile(fileZip.file_path, Path.GetFileName(fileZip.file_name));
                }
            }
            return new PhysicalFileResult(pathsave, FileCommon.GetContentType(pathsave))
        ...
        public async Task<ActionResult> upload_file()
        {
            ...
            foreach (var item in Request.Form.Files)
            {
                ...
                var path = "";
                var count_file = 0;
                path = Path.Combine(_appsetting.folder_path, "file_upload", host, "controller", "id_folder");
                Thread.Sleep(1);
                ...
                var file_path = pathsave.Replace(Path.Combine(_appsetting.folder_path, "file_upload", host), "");
        ...
        /// <summary>
        /// download hàm này được gắn vào khi upload image
        /// </summary>
        ...
        public async Task<ActionResult> Download(string filename, string name)
        {
            ...
            var path = Path.Combine(current, "file_upload", host) + filename;
            ...
            return File(memory, GetContentType(path), Path.GetFileName(name));
        }
        ...
        {
            var request = Request;
            sys_file_upload_db file = new sys_file_upload_db();
            ...
            file.create_by = user_id;
            file.update_by = user_id;
            ...
            if (item != null)
            {
                ...
                var path = Path.Combine(currentpath, "file_upload", host, "image_upload", controller, id_parent, user_id);
                if (!Directory.Exists(path))
                {
                    Directory.CreateDirectory(path);
                }
                ...
                file.file_type = item.ContentType;
                file.status_del = 1;
                file.file_size = item.Length;
                return Json(new { location = file });
            }
