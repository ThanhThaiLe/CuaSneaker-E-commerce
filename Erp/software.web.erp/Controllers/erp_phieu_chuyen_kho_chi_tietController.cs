...
{
    public partial class erp_phieu_chuyen_kho_chi_tietController : common.ControllerBase.BaseAuthenticationController
    {
        private erp_phieu_chuyen_kho_chi_tiet_repo repo;
        private AppSetting _appsetting;
        public erp_phieu_chuyen_kho_chi_tietController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
            repo = new erp_phieu_chuyen_kho_chi_tiet_repo(context);
            _appsetting = appsetting.Value;
        }
        [HttpPost]
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            var model = JsonConvert.DeserializeObject<erp_phieu_chuyen_kho_chi_tiet_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                DTResult<erp_phieu_chuyen_kho_chi_tiet_model> result = new DTResult<erp_phieu_chuyen_kho_chi_tiet_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
        ...
        public ActionResult downloadTemp()
        {
            ...
            string file = newpath + "\\erp_phieu_chuyen_kho_chi_tiet.xlsx";
            byte[] fileBytes = System.IO.File.ReadAllBytes(file);
            System.IO.File.WriteAllBytes(file, fileBytes);
            MemoryStream ms = new MemoryStream(fileBytes);
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, "erp_phieu_chuyen_kho_chi_tiet.xlsx");
        }
        [HttpPost]
        public async Task<IActionResult> ImportFormExcel()
        {
            ...
            if (file.Length > 0)
            {
                ...
                try
                {
                    ...
                    var list_model = new List<erp_phieu_chuyen_kho_chi_tiet_model>();
                    for (int i = 0; i < list_row.Count(); i++)
                    {
                        var itemForm = list_row[i].list_cell.ToList();
                        var model = new erp_phieu_chuyen_kho_chi_tiet_model();
        ...
        public async Task<FileStreamResult> exportToExcel(string search, int status_del)
        {
            ...
            search = search ?? "";
            var query = repo.FindAll().Where(q => q.db.status_del == status_del).OrderByDescending(q => q.db.update_date);
