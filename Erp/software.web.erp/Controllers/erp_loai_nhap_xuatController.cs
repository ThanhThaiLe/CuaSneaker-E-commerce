...
{
    public partial class erp_loai_nhap_xuatController : common.ControllerBase.BaseAuthenticationController
    {
        private erp_loai_nhap_xuat_repo repo;
        private AppSetting _appsetting;
        public erp_loai_nhap_xuatController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
            repo = new erp_loai_nhap_xuat_repo(context);
            _appsetting = appsetting.Value;
        }
        public IActionResult getListUse()
        {
            var result = repo._context.erp_loai_nhap_xuats.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.name,
            }).ToList();
        ...
        {
            var model = JsonConvert.DeserializeObject<erp_loai_nhap_xuat_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                DTResult<erp_loai_nhap_xuat_model> result = new DTResult<erp_loai_nhap_xuat_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
        ...
        public ActionResult downloadTemp()
        {
            ...
            string file = newpath + "\\erp_loai_nhap_xuat.xlsx";
            byte[] fileBytes = System.IO.File.ReadAllBytes(file);
            System.IO.File.WriteAllBytes(file, fileBytes);
            MemoryStream ms = new MemoryStream(fileBytes);
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, "erp_loai_nhap_xuat.xlsx");
        }
        [HttpPost]
        public async Task<IActionResult> ImportFormExcel()
        {
            ...
            if (file.Length > 0)
            {
                ...
                try
                {
                    ...
                    var list_model = new List<erp_loai_nhap_xuat_model>();
                    for (int i = 0; i < list_row.Count(); i++)
                    {
                        var itemForm = list_row[i].list_cell.ToList();
                        var model = new erp_loai_nhap_xuat_model();
