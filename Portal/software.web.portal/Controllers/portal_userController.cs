...
{
    public partial class portal_userController : BaseAuthenticationController
    {
        private portal_user_repo repo;
        private IMailService _mailService;
        private AppSetting _appsetting;
        public portal_userController(IUserService userService, IMailService mailService, SoftwareDefautTable context, IOptions<AppSetting> appsetting, IMapper mapper) : base(userService)
        {
            repo = new portal_user_repo(context, mapper);
        ...
        {
            var model = JsonConvert.DeserializeObject<portal_user_authentication_model>(json.GetValue("data").ToString());
            ...
            var token = tokenHandler.(tokenDescriptor);
        ...
        {
            var model = JsonConvert.DeserializeObject<portal_user_register_model>(json.GetValue("user").ToString());
            ...
            var item = new portal_user_model();
            ...
            item.full_name = model.last_name + " " + model.first_name;
            item.email = model.email;
            item.user_name = model.email;
            item.status_del = 3;
            item.type = 1;
            item.create_date = DateTime.Now;
            item.update_by = getUserId();
            ...
            body = body.Replace("@@user_name@@", item.full_name);
            body = body.Replace("@@url_update_status@@", "https://" + Request.Host.Value + "/sys_user.ctr/update_status_del?email=" + item.email + "&status=" + 1);
