...
{
    public partial class sys_don_vi_tinhController : common.ControllerBase.BaseAuthenticationController
    {
        private sys_don_vi_tinh_repo repo;
        private AppSetting _appsetting;
        public sys_don_vi_tinhController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
        ...
        {
            var result = repo._context.sys_don_vi_tinhs.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.name,
            }).ToList();
        ...
        public ActionResult downloadTemp()
        {
            ...
            string file = newpath + "\\sys_don_vi_tinh.xlsx";
            byte[] fileBytes = System.IO.File.ReadAllBytes(file);
            System.IO.File.WriteAllBytes(file, fileBytes);
            MemoryStream ms = new MemoryStream(fileBytes);
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, "sys_don_vi_tinh.xlsx");
        }
        [HttpPost]
        public async Task<IActionResult> ImportFormExcel()
        {
            ...
            if (file.Length > 0)
            {
                ...
                try
                {
                    ...
                    var list_model = new List<sys_don_vi_tinh_model>();
                    for (int i = 0; i < list_row.Count(); i++)
                    {
                        var itemForm = list_row[i].list_cell.ToList();
                        var model = new sys_don_vi_tinh_model();
