...
{
    public partial class sys_dieu_khoanController : common.ControllerBase.BaseAuthenticationController
    {
        private sys_dieu_khoan_repo repo;
        private AppSetting _appsetting;
        public sys_dieu_khoanController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
            repo = new sys_dieu_khoan_repo(context);
            _appsetting = appsetting.Value;
        }
        public IActionResult getListUse()
        {
            var result = repo._context.sys_dieu_khoans.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.name,
                link = q.link,
        ...
        {
            var model = JsonConvert.DeserializeObject<sys_dieu_khoan_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var dataList = await Task.Run(() => query.OrderByDescending(q => (q.db.stt ?? 0)).Skip(param.Start).Take(param.Length).ToList());
                DTResult<sys_dieu_khoan_model> result = new DTResult<sys_dieu_khoan_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
        ...
        public ActionResult downloadTemp()
        {
            ...
            string file = newpath + "\\sys_dieu_khoan.xlsx";
            byte[] fileBytes = System.IO.File.ReadAllBytes(file);
            System.IO.File.WriteAllBytes(file, fileBytes);
            MemoryStream ms = new MemoryStream(fileBytes);
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, "sys_dieu_khoan.xlsx");
        }
        [HttpPost]
        public async Task<IActionResult> ImportFormExcel()
        {
            ...
            if (file.Length > 0)
            {
                ...
                try
                {
                    ...
                    var list_model = new List<sys_dieu_khoan_model>();
                    for (int i = 0; i < list_row.Count(); i++)
                    {
                        var index = -1;
                        var itemForm = list_row[i].list_cell.ToList();
                        var model = new sys_dieu_khoan_model();
                        var stt = itemForm[index++].value.ToString().Trim() ?? "";
                        var code = itemForm[index++].value.ToString().Trim() ?? "";
                        var name = itemForm[index++].value.ToString().Trim() ?? "";
                        var link = itemForm[index++].value.ToString().Trim() ?? "";
                        var so_thu_tu = itemForm[index++].value.ToString().Trim() ?? "";
                        var note = itemForm[index++].value.ToString().Trim() ?? "";


                        // map data
                        model.db.stt = so_thu_tu == "" ? 0 : int.Parse(so_thu_tu);
                        model.db.link = link;
                        model.db.code = code;
                        model.db.name = name;
                        model.db.note = note;
