...
{
    public partial class sys_loai_san_phamController : common.ControllerBase.BaseAuthenticationController
    {
        private sys_loai_san_pham_repo repo;
        private AppSetting _appsetting;
        public sys_loai_san_phamController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
        ...
        {
            var result = repo._context.sys_loai_san_phams.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.name,
            }).ToList();
        ...
        public ActionResult downloadTemp()
        {
            ...
            string file = newpath + "\\sys_loai_san_pham.xlsx";
            byte[] fileBytes = System.IO.File.ReadAllBytes(file);
            System.IO.File.WriteAllBytes(file, fileBytes);
            MemoryStream ms = new MemoryStream(fileBytes);
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, "sys_loai_san_pham.xlsx");
        }
        [HttpPost]
        public async Task<IActionResult> ImportFormExcel()
        {
            ...
            if (file.Length > 0)
            {
                ...
                try
                {
                    ...
                    var list_model = new List<sys_loai_san_pham_model>();
                    for (int i = 0; i < list_row.Count(); i++)
                    {
                        var itemForm = list_row[i].list_cell.ToList();
                        var model = new sys_loai_san_pham_model();
