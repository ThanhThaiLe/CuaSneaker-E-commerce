...
{
    public partial class sys_template_mailController : common.ControllerBase.BaseAuthenticationController
    {
        private sys_template_mail_repo repo;
        private AppSetting _appsetting;
        public sys_template_mailController(IUserService userService, SoftwareDefautTable context, IOptions<AppSetting> appsetting) : base(userService)
        {
        ...
        {
            var result = repo._context.sys_template_mails.Where(q => q.status_del == 1).Select(q => new sys_common_number_model()
            {
                id = q.id,
                name = q.name,
            }).ToList();
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var query = repo.FindAll()
                    ...
                    .Where(q => q.db.name.Contains(search) || search == "");
